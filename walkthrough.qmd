...
```{python}
installation = !pip install seer-pas-sdk
print(installation)
```
...
You may also choose to pass in an `instance` param in the SDK object to instantiate the PAS SDK to the EU or US instance. As an example for this tutorial, we will define the instance as `dev`:
```{python}
INSTANCE = "dev"
```

### Instantiation
By default, if you don't specify an instance, the PAS SDK will be instantiated with the US instance. After importing the SeerSDK module, you can instantiate an object in the following way:
```{python}
...
* `project_id`: (`str`) ID of the project to which the analysis belongs. Can be fetched using the `get_project_metadatas()` function.
* `sample_ids`: (`list[str]`, optional): List of sample IDs to be used for the analysis. Must always belong to the project being chosen to start an analysis with. Should be omitted if analysis is to be run with all samples.
...
###### <u>Example</u>
Since no `sample_ids` are specified, the analysis is run on all samples.
```{python}
name = "testAnalysis"
...
log(sdk.start_analysis(name, project_id, analysis_protocol_name=analysis_protocol_name))
```

You can also specify select samples using the following:
```{python}
name = "testAnalysis"
project_id = "1e324300-0496-11ee-bd19-a77197cd1a6b"
sample_ids = ["1b1ff390-048e-11ee-bd19-a77197cd1a6b"] # this sample id is contained in the project
analysis_protocol_name = "testMSFraggerUpload26"
log(sdk.start_analysis(name, project_id, sample_ids=sample_ids, analysis_protocol_name=analysis_protocol_name))
```
<hr>

However, if the sample specified are invalid, the code will throw an error.
```{python}
name = "testAnalysis"
project_id = "1e324300-0496-11ee-bd19-a77197cd1a6b"
sample_ids = ["ABC1234"] # sample not contained in the project
analysis_protocol_name = "testMSFraggerUpload26"
log(sdk.start_analysis(name, project_id, sample_ids=sample_ids, analysis_protocol_name=analysis_protocol_name))
```
